  ...
  for (const page in pages) {
    ...
    if (hasKeywords && hasKeysentences) {
      if (!atLeastOneUniqueKeyOnPage) {
        throw Error(`Found several occurences of all keywords on page ${page}. Either use keywords where at least one is unique per document, or check if document contains several or overlapping documents on page ${page}.`)
      } else {
